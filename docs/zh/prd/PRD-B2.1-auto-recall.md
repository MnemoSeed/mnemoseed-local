...
- → 例外（2026-09-12，issue #187 范围裁剪，非相关性预审）：当调用方给出当前用户原文 `resume_query` 时，可用确定性标识符锚点做范围过滤——回答"哪些会话与当前项目同源"而非"该记忆是否相关"。无锚点或无命中时不注入（`selection: unresolved`），有 `resume_query` 缺席时标记 `unscoped`；此为例外，不改变无外部线索时的默认时近语义。
