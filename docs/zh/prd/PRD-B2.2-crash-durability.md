...
- **T1 hook 水位线（ack 钟，非发送钟）**：水位文件族 `hook-watermarks*.json`（`{session_id: last_acked_ts}`，仍算**单一可删除机制**：删掉整个文件族即回到无回放原型）——推钟只发生在 POST 收到 daemon **2xx** 的回执里（fire-and-forget 的 `.then` 链），发出≠到达；宕机期间水位停摆在最后被确认的轮次，重叠窗口绝不会吞掉未达内容。Legacy `hook-watermarks.json` 对新代码**只读**（旧版本的全量覆写文件，原样保留、纳入每次合并、永不覆写）；新代码只写自己的独占分片 `hook-watermarks.<pid>.<uuid>.json`（进程自有 tmp 名 `.<分片>.tmp.<uuid>.<counter>` 原子 write+rename，Windows 瞬态 `EPERM/EACCES/EBUSY` 有界重试 N≤5，失败只记 debug，主 lane 永不阻塞/抛错；进程内 persist 串行化，调用永不 reject）。每次刷写（含初次加载）先枚举 legacy + 全部分片完整快照（严格谓词：legacy 全名精确匹配或 `hook-watermarks.<pid>.<uuid>.json` 正则，任何 tmp/近似名一律排除）、只接受**有限非负数**值、逐 key 取 max 与内存缓存合并，再落盘自己分片（原子替换保证单文件完整，源分片保留至安全 GC，故并发读写不丢 key）。ack 钟入口同样只接受有限非负数（`NaN`/`Infinity`/负数在 `noteWatermark` 直接拒绝）；首次加载前的有效 ack 先在空原型暂存表累积，加载把磁盘最大值合并进该表（永不替换丢弃）。同一次串行 persist 内若有新 note 落在写窗口之后，最多做 2 次有界跟随重写；落在最后一次写窗口内的 note 留在缓存由下次 cadence 刷写（仍在至多一轮边界内）。刷写点 = 各 cadence 点（idle/error/deleted/compacting），读写故障一律吞没降级，绝不侵入主链路。
- **T1-补充 tmp 有界（确切行为）**：自有 tmp 写失败/改名失败立即尽力删除；每次串行 persist 开头清扫**确切自前缀**（`.<自分片名>.tmp.` 开头，本实例）残留，最多 20 个、尽力而为；跨实例仅删同时满足三者的一律视为可删：与 `.<pid>.<uuid>.json.tmp.<uuid>.<counter>` 精确格式逐字匹配、年龄 >24h、编码 PID 经 `ESRCH` 确认已死（存活/`EPERM`/未知一律跳过），每次最多 20 个。Legacy `Date.now` 格式 tmp、无关名、存活/不明进程的 tmp 永不触碰。腐坏终态文件永不删除（见下）。
