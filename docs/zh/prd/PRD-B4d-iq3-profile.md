...
| 16GB 第三选项 (候选) | `mnemoseed-local/qwen3.5-9b-iq3xs` | IQ3_XS (~3.3 bpw) | `https://huggingface.co/bartowski/Qwen_Qwen3.5-9B-GGUF`（base `Qwen/Qwen3.5-9B`，量化自观 `bartowski`，Apache-2.0；文件名 `Qwen_Qwen3.5-9B-IQ3_XS.gguf`） | `182be2fd6c7bc44887d88a91cb03ff009cc9f549`（HF repo HEAD；文件无 own LFS oid，oid=120ef958） | `120ef957b56c90668993d4ea2220ca8608a4b0e48874f3e509ee0c912a330eb9`（Get-FileHash 实测匹配 HF sha256） | **4.82 GB**（Get-ChildItem 实测 4,590,080 字节 ≈4.49 GiB） | dev 机 3800X 纯 CPU 实测：9B Q4 基线 ~3.5 tok/s（JWProbeA) 推算 IQ3_XS（4.5GB 小权重）略高于 ~4–5 tok/s；**均未达 bar ≥15 tok/s** → PRD §4 判定树"速度塌→记为硬件要求，不自动否决" | 待办公机实测：KV@16k(q8_0) ~1.3GB + embedder ~2GB 理论可容 16GB；dev 机 (32GB+RTX3070) 非目标画像，见 §8 硬件门 | 首跑 canary-00 (GPU, off): recall 0.38 / pollution 3 / core 8 / tokens 1692,无坍缩;见 §10 |
