...
- **daemon on/off 用户可控开关（B2.5，2026-08-20 用户立项，B2.4 收口后为当前批次）**：command 直接关停 daemon（不让 agent 使用记忆）并持久禁用——`off` = 优雅关停 + 哨兵文件 `CONFIG_DIR/daemon.off`（supervision/`up` 不再拉起，agent 侧诚实报"已关闭"而非催促 `up`）；`on` = 解除禁用并启动；安装后默认开。设计定案已就位（solution-architect SHIP-WITH-ADJUSTMENTS：loopback `POST /daemon/shutdown` respond-then-exit、watchdog 先 `disarm()` 再 `request_shutdown()`、哨兵文件在 configwrite/DB-primary 机制之外——避开离线写入被 DB 覆写复活与 version_id 槽位移、网关 disabled 文案纯增补不碰既有 pin、理论锚"无借用——工程控制面"）。
