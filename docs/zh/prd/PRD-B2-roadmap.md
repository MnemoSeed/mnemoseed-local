...
- **B2.2 崩溃耐久**：2026-08-19 立项（PRD-B2.2-crash-durability，KISS 单机制——宿主会话史重生回放，ack 水位 + per-session FIFO 链）。首轮 QA 复审 NOT CLOSABLE（2 BLOCKER：发送钟水位、live/replay 到达序错绑），已修复并配 node 行为挂架钉死；收口流程进行中。
