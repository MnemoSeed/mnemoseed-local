...
- **E0 体验通道基础（2026-09-11 收口，squash commit `485c9bf` → issue #151；E0 里程碑此前仅文档，本批为核心实现）**：#151 遗留 hygiene 项落地：`composite_group_id` 载体（v13 迁移，legacy NULL 回填 + 空白归一化）；读侧证据命运解析——`evidence_retired` 字段（CHUNK 指针面对已清除 chunk 返回 `true`，其余 `null`）；`dream.experience_channel.enabled` 默认关闭标志（config 注册表 + tripwire 断言零热路径消费）；所有 5 面 version pin 12→13 升级（parity/meta/graph/B1/contract）。附带修复：`_dream_apply` 原丢失 `reflect_batch_max_tokens` + `experience_channel`，现已携带。红线 6 完全兑现：chunk_ids IN 子句限缩守卫通过复合双源 oracle 测试（活 + 不存在的 chunk 在同一页内独立解析）。门禁 2607 passed / 5 skipped；QA CLOSABLE（0 BLOCKER）。
