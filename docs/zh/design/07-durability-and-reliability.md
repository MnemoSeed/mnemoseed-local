...
1. **ack 水位（ack 钟，非发送钟）**：`plugin.ts` 的 `noteWatermark` 只在 POST 收到 daemon **2xx** 回执时推进，推钟路径全部经 `post()` 的 2xx 分支。发出≠到达——fire-and-forget 语义下若按发送钟推水位，宕机会静默吞掉未达内容（B2.2 BLOCKER-1 已修）。水位文件族 `hook-watermarks*.json` 仍算**单一可删除机制**：legacy `hook-watermarks.json` 对新代码只读（永不覆写，纳入每次合并）；新代码只写自己的独占分片 `hook-watermarks.<pid>.<uuid>.json`（进程自有 tmp 名含 pid+uuid+单调 counter，原子 write+rename，Windows 瞬态 `EPERM/EACCES/EBUSY` 有界重试 N≤5、总时限有界，失败只记 debug；进程内 persist 串行化，调用永不 reject）。初次加载与每次 persist 都先按严格谓词枚举 legacy + 全部分片完整快照（legacy 全名或终态分片正则，tmp/近似名排除）、只接受有限非负数值、逐 key 取 max 与内存合并，再落盘自己分片（原子替换下读者只见旧/新完整快照之一，源分片保留至安全 GC，故并发不丢 key）。ack 钟入口拒绝非有限非负时间戳；首载前的有效 ack 在空原型暂存表累积，加载合并磁盘值进该表而不替换。同次 persist 内写窗口后到达的 note 触发至多 2 次有界跟随重写，落在最后写窗口内的残留由下次 cadence 刷写（至多一轮边界内）。刷写点 = 各 cadence 点（idle/error/deleted/compacting）；读写故障一律吞没降级，绝不侵入主链路。tmp 有界：失败的自有 tmp 立即尽力删，每次 persist 开头清扫确切自前缀残留（≤20）；跨实例只删精确 crash-tmp 格式、年龄 >24h、PID 经 `ESRCH` 确认已死者（≤20）；legacy `Date.now` tmp 与无关名永不触碰。腐坏冻结：非法文件/key 跳过、原文件保留、仅记 debug 元数据。保守 GC：分片年龄 >24h 且 PID 经 `ESRCH` 确认已死才合规（存活/`EPERM`/未知/复用歧义跳过），先把全局最大值落盘成功后再删最多 20 个；永不删 legacy、腐坏文件、存活/不确定分片或他进程 tmp。混跑（旧+新进程共存）降级至全宿主重启才收敛，记为残留。
