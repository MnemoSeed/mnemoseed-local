...
```
flowchart TD
    A[daemon 2xx 回执] -->|ack 钟推进| W[(hook-watermarks.json<br/>tmp+rename 原子写)]
...
1. **ack 水位（ack 钟，非发送钟）**：`plugin.ts` 的 `noteWatermark` 只在 POST 收到 daemon **2xx** 回执时推进（`plugin.ts:616`），推钟路径全部经 `post()` 的 2xx 分支（`plugin.ts:117`）。发出≠到达——fire-and-forget 语义下若按发送钟推水位，宕机会静默吞掉未达内容（B2.2 BLOCKER-1 已修）。刷写 = 各 cadence 点（idle/error/deleted/compacting）原子落盘 `hook-watermarks.json`：`persistWatermarks` 先写唯一后缀 tmp 文件再 `rename`（`plugin.ts:625-634`），崩溃撕文件不毁上一好版本；读写故障一律吞没降级，绝不侵入主链路。
