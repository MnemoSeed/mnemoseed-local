...
- → 设计规则：钉住条目获得独立慢衰减档 λ_pin（量级取偏好档，开放问题 1）。显著性信号 = 用户显式钉入这个动作本身（元记忆里的最高权威信号，Nelson & Narens 1990，本地 R36 ✅），不是情绪评分——捕获中立红线原样保留：系统不得自行给内容判显著性喂给巩固。档位只调持久性，永不触碰 provenance.confidence。
...
- → 设计规则：候选准入从单一硬地板改为两级——主池照旧（权重 ≥ 0.4 无条件入池），救援带内的钉住类条目凭足够强的线索匹配度也可入池，排名靠后即可。融合公式不变（score = α·语义 + β·线索 + γ·权重 + δ·中心度，`retrieve/hybrid.py:365-386`）：弱痕迹靠线索分量入池、靠总分排后，不需要新排序逻辑。阈值数值属机制层标定（开放问题 3）。
...
设计：同主题带内（近重复探针以现有冲突阈命中，`memory.py:546`）三分支收口到同一实体——

| 子情形 | 行为 | 现状对照 |
|---|---|---|
| 强相似 ∧ 文本一致 | 就地回弹刷新（不变） | `memory.py:556-572` |
| 文本判定冲突 | 在同一实体上标 needs_reconcile（不变）——真正的说法矛盾留给显式裁决，不静默覆盖 | `memory.py:573-587` |
| 其余（换措辞的修订） | **新增**：就地替换——同 chunk_id upsert 新 verbatim 文本 + 权重回弹刷新 + 向既有 `provenance.history` 追加一条 `ProvenanceEvent(action="superseded", ...)` | 现状 fall-through 成第二条 |
...
- 渲染位置（MCP recall 显式响应 vs T2 自动注入）= 开放问题 2；
...
| 预过滤放宽多捞候选 | 救援带宽度 × 钉住数量的有界增量 | top-K 截断限界；评测臂盯召回质量与延迟 |
...
1. **慢档速率**：直接取偏好档 0.005（≈139 天半衰期），还是取介于其间的值（如 0.0075 ≈ 92 天）？前者有"沿用已标定档位"的简洁性；后者承认钉住的显著性可能高于一般偏好。默认推荐 0.005。
2. **残迹渲染位置**：只进 MCP recall 显式响应，还是也进 T2 自动注入？后者每轮都可能见到残迹行（打扰成本），前者依赖主动性。默认推荐仅 MCP recall，dogfood 后复议。
3. **救援带参数**：救援地板初值（如 0.25）与线索匹配阈值下限（β 分量的门槛，如 0.3）——机制层标定项，评测臂数据说话，本设计不预设神圣值。
