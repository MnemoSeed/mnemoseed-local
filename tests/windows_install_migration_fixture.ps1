...
$trigger = [pscustomobject]@{ CimClass = [pscustomobject]@{ CimClassName = 'MSFT_TaskLogonTrigger' }; UserId = $env:USERNAME }
$task = [pscustomobject]@{
...
& $TaskScript -BootstrapSource ([IO.Path]::GetTempFileName()) -TaskName 'MnemoSeedLocalDaemon' -User $env:USERNAME -MigrateLegacyOllamaTask -TaskLookup $lookup -TaskUnregister $unregister -MigrationResult $Result
exit 0
